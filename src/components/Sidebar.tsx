...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onSelectView, onOpenSusesoManual }) => {
  ...
  const licenseAlertCount = drivers.filter((d) => {
    if (!d.licenseExpiry) return false;
    const expiry = new Date(d.licenseExpiry);
    const now = new Date('2026-09-13');
    const days = Math.ceil((expiry.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
    return days <= 30; // Vencida o por vencer en 30 días o menos
  }).length;
  ...
  const navItems: {
    ...
    {
      id: 'institutional_web',
      label: 'Web Institucional (Oficial)',
      rfCode: 'PORTAL',
      icon: Globe,
      badge: 'PÚBLICA',
    ...
    {
      id: 'prevention_protocols',
      label: 'Protocolos de Prevención',
      rfCode: 'INTERVENCIÓN',
      icon: ShieldAlert,
      badge: 'SUSESO',
      badgeColor: 'bg-blue-600 text-white'
    ...
    { id: 'drivers_fleet', label: 'Conductores y Flota', rfCode: 'RF-003/4', icon: Users },
    {
      ...
      label: 'Validador de Licencias',
      rfCode: 'LEY 18.290',
      icon: FileCheck2,
      badge: licenseAlertCount > 0 ? licenseAlertCount : undefined,
      badgeColor: 'bg-rose-500 text-white'
    },
    {
      id: 'checklists',
      label: 'Plantillas de Checklists',
      rfCode: 'PRE-USO',
      icon: ClipboardCheck,
      badge: 'NUEVO',
      badgeColor: 'bg-emerald-600 text-white'
