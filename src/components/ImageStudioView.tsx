...
import { generateSafetyGraphic } from '../utils/safetyGraphicGenerator';
import {
  Sparkles,
  Image as ImageIcon,
  Wand2,
...
export const ImageStudioView: React.FC = () => {
  ...
  const handleGenerateOrEdit = async () => {
    if (!prompt.trim()) {
      setErrorMessage('Por favor ingresa una descripción o instrucción para la imagen.');
      return;
    }
    ...
    const activeCompany = currentCompany.fantasyName || currentCompany.businessName || 'Blindaje Vial SpA';
    ...
    try {
      ...
      if (data.isQuotaExceeded || data.warning) {
        setQuotaWarning(
          data.warning ||
          'La cuota de la API de generación de imágenes de Gemini se ha superado temporalmente (Error 429 RESOURCE_EXHAUSTED). Se activó automáticamente el motor gráfico vectorial oficial para generar su pieza técnica en alta resolución.'
        );
      }
      ...
      if (data.imageUrl) {
        const newItem: GeneratedImageItem = {
          ...
          category: data.category || (mode === 'edit' ? 'Edición' : 'Generación Directa'),
          isVector: Boolean(data.isFallback)
        };

        setCurrentResult(newItem);
        setGallery((prev) => [newItem, ...prev]);
      } else {
        throw new Error(data.error || 'No se pudo generar la imagen con el servicio de IA.');
      }
    } catch (err: any) {
      console.warn('API error or quota limit reached; generating client vector safety graphic:', err);
      
      // Client-side fallback generator: never leave user stranded
      const fallback = generateSafetyGraphic({
        prompt: prompt.trim(),
        companyName: activeCompany,
        aspectRatio,
        stylePreset
      ...
      setGallery((prev) => [newItem, ...prev]);
      setQuotaWarning(
        'Modo Contingencia Activo: Se generó el diseño vectorial oficial debido a que la cuota de la API de Gemini se ha superado temporalmente (429 RESOURCE_EXHAUSTED). El activo está disponible para visualización, descarga e impresión inmediata.'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  // Direct 1-click generator without consuming any AI tokens
  const handleGenerateVectorDirect = () => {
    if (!prompt.trim()) {
      setErrorMessage('Por favor escribe una descripción o selecciona una plantilla para generar el gráfico.');
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-lg space-y-5">
            ...
            {/* Quota & Contingency Notification */}
            {quotaWarning && (
              <div className="bg-amber-950/40 border border-amber-500/40 p-4 rounded-xl text-xs space-y-2 text-amber-200">
                <div className="flex items-center gap-2 font-bold text-amber-300">
                  <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0" />
                  <span>Modo de Contingencia Activo (Aviso de Cuota)</span>
                </div>
                <p className="text-[11px] leading-relaxed text-amber-200/90">
                  {quotaWarning}
                </p>
                <div className="pt-1 text-[10px] text-amber-300/70 flex items-center gap-1.5 border-t border-amber-800/40">
                  <Info className="w-3.5 h-3.5 shrink-0" />
                  <span>
                    El activo generado en SVG vectorial es 100% nítido e imprimible. Para habilitar fotorrealismo IA ilimitado, puede configurar su propia clave en <strong>Ajustes &gt; Secrets (GEMINI_API_KEY)</strong>.
                  </span>
                </div>
              </div>
            )}
              ...
              <div className="bg-rose-950/50 border border-rose-800 text-rose-300 p-3.5 rounded-xl text-xs flex items-start gap-2">
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-[10px] text-rose-300/80">
                    Puedes usar el botón "Generar Gráfico Vectorial" para obtener tu señalética de inmediato sin depender de cuota externa.
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 disabled:opacity-60 text-white font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-blue-600/30 transition cursor-pointer text-xs sm:text-sm"
              >
                {isLoading ? (
                  <>
                  ...
                  <>
                    ...
                    <span>{mode === 'create' ? 'Generar con Gemini IA' : 'Editar con Gemini'}</span>
                  </>
                )}
              </button>

              {/* Direct Vector Generator Button (Instant & Zero Quota) */}
              <button
                type="button"
                onClick={handleGenerateVectorDirect}
                disabled={isLoading}
                className="w-full bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-blue-500 text-slate-100 font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 shadow-md transition cursor-pointer text-xs sm:text-sm"
                title="Genera señalética e infografías industriales oficiales directamente en formato vectorial SVG sin consumir cuota"
              >
                <Zap className="w-4 h-4 text-amber-400" />
                <span>Generar Vectorial (Sin Cuota)</span>
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              {currentResult && (
                <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${
                  currentResult.isVector
                    ? 'bg-amber-500/20 text-amber-300 border-amber-500/30'
                    : 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30'
                }`}>
                  {currentResult.isVector ? 'VECTORIAL HD' : 'GEMINI 3.1 IA'}
                </span>
              )}
            </div>
          ...
          {gallery.map((item) => (
            ...
            >
              <div className="relative h-40 bg-slate-900 overflow-hidden">
                ...
                />
                <span className="absolute top-2 left-2 bg-slate-950/80 backdrop-blur-xs text-[10px] font-bold text-blue-300 px-2 py-0.5 rounded border border-slate-800 flex items-center gap-1">
                  {item.isVector && <Zap className="w-2.5 h-2.5 text-amber-400" />}
