...
import { DriverLicenseValidator } from './DriverLicenseValidator';
import {
  Users,
  Truck,
  PlusCircle,
...
export const DriversAndFleetView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'drivers' | 'fleet' | 'license_validator'>('drivers');
  const [searchTerm, setSearchTerm] = useState('');
  const [filterBase, setFilterBase] = useState('all');

  const urgentLicensesCount = drivers.filter((d) => {
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 p-1 rounded-xl">
          ...
          <button
            onClick={() => setActiveTab('license_validator')}
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-bold transition cursor-pointer ${
              activeTab === 'license_validator'
                ? 'bg-blue-600 text-white shadow'
                : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <FileCheck2 className="w-4 h-4" />
            <span>Validador de Licencias</span>
            {urgentLicensesCount > 0 && (
      ...
      {activeTab === 'drivers' && (
        <div className="space-y-6">
          {/* Quick Alert Banner for Driver Licenses */}
          {urgentLicensesCount > 0 && (
            <div className="p-4 rounded-xl border border-rose-500/50 bg-rose-950/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-white">
                    ¡Alerta Operativa! Se detectaron {urgentLicensesCount} {urgentLicensesCount === 1 ? 'licencia crítica o próxima a vencer' : 'licencias críticas o próximas a vencer'} (&le; 30 días)
                  </h4>
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    Validación continua contra Ley 18.290 de Tránsito. Se recomienda verificar la vigencia antes de autorizar despachos.
                  </p>
                </div>
              </div>
              <button
                onClick={() => setActiveTab('license_validator')}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg transition shrink-0 cursor-pointer shadow"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredDrivers.map((driver) => {
              ...
              const isLicenseExpired = daysRemaining < 0;
              const isLicenseUrgent = daysRemaining >= 0 && daysRemaining <= 15;
              const isLicenseWarning = daysRemaining > 15 && daysRemaining <= 30;
                ...
                >
                  <div>
                    ...
                    <div className="mt-3.5 space-y-1.5 text-xs">
                      ...
                      <div className="flex items-center justify-between text-slate-300 bg-slate-800/40 p-2 rounded-lg">
                        <span className="text-slate-400 text-[11px]">Licencias:</span>
                      ...
                      {/* Expiración Licencia Ley 18.290 */}
                      <div className={`flex items-center justify-between p-2 rounded-lg border transition ${
                        isLicenseExpired
                        ...
                        <div className="flex items-center gap-1.5">
                          <FileCheck2 className={`w-3.5 h-3.5 ${
                            isLicenseExpired || isLicenseUrgent ? 'text-rose-400' : isLicenseWarning ? 'text-amber-400' : 'text-blue-400'
                          }`} />
                          <span className="text-[11px] font-semibold">Venc. Licencia:</span>
                        </div>
                        <div className="text-right">
                          <span className="font-mono text-[11px] font-bold block">{driver.licenseExpiry || 'No registrada'}</span>
                          <span className={`text-[10px] font-bold ${
                            ...
                            {isLicenseExpired
                              ? `Vencida (${Math.abs(daysRemaining)}d)`
                              : isLicenseUrgent
                              ? `¡Vence en ${daysRemaining}d!`
                              : isLicenseWarning
                              ? `Próxima (${daysRemaining}d)`
                              : `${daysRemaining}d vigentes`}
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-semibold bg-slate-800/90 hover:bg-slate-700 text-slate-200 border border-slate-700 transition cursor-pointer"
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer ${
                          isSelectedForRadar
                            ? 'bg-blue-600 text-white shadow'
                      ...
                      <button
                        onClick={() => {
                          setActiveTab('license_validator');
                        }}
                        className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-semibold bg-slate-800/90 hover:bg-slate-700 text-indigo-300 border border-slate-700 transition cursor-pointer"
                        title="Validar vigencia de licencia en registro oficial"
                      >
      ...
      {/* Content: Driver License Validator Tab */}
      {activeTab === 'license_validator' && (
