...
import { SupervisorPushNotificationCenter } from './notifications/SupervisorPushNotificationCenter';
import {
  ShieldCheck,
  AlertTriangle,
  Bell,
    ...
    <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-40 px-4 py-2.5">
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Quick Institutional Website Button */}
          {onNavigate && (
            <button
              onClick={() => onNavigate('institutional_web')}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-gradient-to-r from-blue-600/30 to-indigo-600/30 hover:from-blue-600/40 hover:to-indigo-600/40 text-blue-200 border border-blue-500/50 transition cursor-pointer shadow-sm"
              title="Ver Sitio Web Público de la Institución (Blindaje Vial SpA)"
            >
              <Globe className="w-3.5 h-3.5 text-blue-400" />
              <span className="hidden sm:inline font-bold">Web Institucional</span>
              <span className="hidden lg:inline text-[9px] bg-blue-500/30 text-blue-100 px-1 py-0.2 rounded font-mono">
                PÚBLICA
              </span>
            </button>
          )}
          ...
          {/* Real-time Supervisor Push Notification System (Ley 18.290 / SUSESO) */}
          <SupervisorPushNotificationCenter onNavigate={onNavigate} />
