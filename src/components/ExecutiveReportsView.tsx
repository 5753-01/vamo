...
import { ExecutiveFloatingPdfButton } from './ExecutiveFloatingPdfButton';
...
export const ExecutiveReportsView: React.FC = () => {
  ...
  const handleDownloadHtml2Pdf = async () => {
    try {
      setIsGenerating('pdf');
      showToast('Generando PDF de Alta Legibilidad...');
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto executive-reports-container">
      ...
      <div className="print-only executive-print-header hidden print:block mb-6 pb-4 border-b-2 border-slate-900">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm executive-screen-header">
        ...
        <div className="flex flex-wrap items-center gap-2 no-print">
          ...
          {/* Browser Direct Print Button with Tooltip and Guide Trigger */}
          <div className="relative inline-flex items-center">
            <button
              ...
              className="flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-xs font-semibold px-3 py-2 rounded-l-xl transition cursor-pointer shadow-sm"
            ...
            <button
              id="print-guide-helper-btn"
              onClick={() => setShowPrintGuideModal(true)}
              className="bg-slate-800 hover:bg-slate-700 hover:text-amber-300 border border-l-0 border-slate-700 text-slate-400 px-2 py-2 rounded-r-xl transition cursor-pointer"
              title="Ver guía y ajustes óptimos para el cuadro de impresión nativo (Gráficos de fondo, A4, márgenes)"
              aria-label="Guía de impresión óptima"
            >
              <HelpCircle className="w-3.5 h-3.5 text-amber-400/90" />
            </button>

            {/* Subtle Print Instruction Tooltip on Hover */}
            {showPrintTooltip && (
              <div className="absolute right-0 top-full mt-2 w-72 bg-slate-950/95 border border-amber-500/40 text-slate-200 text-[11px] rounded-xl p-3 shadow-2xl z-50 pointer-events-none animate-in fade-in zoom-in-95 duration-100">
                <div className="flex items-center gap-1.5 text-amber-300 font-bold mb-1">
                  <Printer className="w-3.5 h-3.5 text-amber-400" />
                  <span>Ajustes Óptimos de Impresión</span>
                </div>
                <p className="text-slate-300 leading-snug mb-2">
                  En el diálogo nativo del navegador, active <strong>«Gráficos de fondo»</strong> y seleccione <strong>A4</strong> para preservar sellos y colores normativos.
                </p>
                <div className="text-[10px] text-amber-400/90 font-mono flex items-center justify-between border-t border-slate-800 pt-1.5">
                  <span>Haga clic en «?» para la guía completa</span>
                  <span className="font-bold">Ctrl+P</span>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="print-only executive-print-footer hidden print:block mt-8 pt-4 border-t-2 border-slate-400 space-y-5">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950 sticky top-0 z-20">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center">
                  <button
                    onClick={() => window.print()}
                    className="flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold px-3 py-2 rounded-l-xl transition cursor-pointer"
      ...
      {/* Print Guide Dialog */}
      <PrintGuideDialog
        isOpen={showPrintGuideModal}
        onClose={() => setShowPrintGuideModal(false)}
        onConfirmPrint={() => window.print()}
      />

      {/* Floating Action Button (FAB) - Generar PDF */}
      <ExecutiveFloatingPdfButton
        reportName="Centro de Reportes Ejecutivos e Indicadores Clave (KPI)"
      />
    </div>
  );
};
