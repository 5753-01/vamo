...
// -------------------------------------------------------------
// Real-Time Supervisor Push Notifications
// -------------------------------------------------------------
...
export type PushNotificationSeverity = 'critica' | 'alta' | 'media' | 'informativa';

export interface RealtimePushNotification {
  id: string;
  ...
  // Specific payload for high-risk tests
  testData?: {
    testId: string;
    testCode: string;
    driverId: string;
  ...
  // Specific payload for driver license expiration
  licenseData?: {
    driverId: string;
    driverName: string;
    driverRut: string;
    ...
    licenseClass: string[];
    licenseExpiry: string;
    daysOverdue: number; // positive if overdue, negative if remaining days
    isExpired: boolean;
    legalArticle: string; // e.g., 'Art. 110-111 Ley 18.290 de Tránsito'
    companyName?: string;
  };
...
// -------------------------------------------------------------
// Prevention Protocols & Intervention Action Engine (Ley 18.290 / SUSESO / RIOHS)
// -------------------------------------------------------------
export type PreventionProtocolResultType =
  | 'negativo'
  | 'positivo_alcohol_infraccion'
  | 'positivo_alcohol_ebriedad'
  | 'presunto_positivo_drogas'
  | 'retest_en_espera'
  | 'rechazo_test'
  | 'invalido_falla';

export type PreventionRiskLevel = 'bajo' | 'moderado' | 'alto' | 'critico';

export type InterventionActionCategory =
  | 'operacional'
  | 'comunicacion_alertas'
  | 'legal_laboral'
  | 'medica_toxicologica';

export interface AutomaticInterventionAction {
  id: string;
  name: string;
  code: string; // e.g., 'ACT-BLOQ-DESP', 'ACT-CUSTODIA-LAB'
  description: string;
  category: InterventionActionCategory;
  enabled: boolean;
  mandatoryByLaw: boolean; // Si la normativa no recomienda apagarla
  legalReference: string; // e.g., 'Art. 115 bis Ley 18.290', 'Art. 184 CT'
  slaResolutionMinutes: number; // e.g. 15, 60, 1440
  assignedRole: RoleType;
  requiresSupervisorAck: boolean;
}

export interface PreventionProtocolConfig {
  id: string;
  resultType: PreventionProtocolResultType;
  title: string;
  subtitle: string;
  badgeLabel: string;
  ...
  alcolockStateRequired: 'habilitado' | 'bloqueo_temporal' | 'bloqueo_total_corte';
  retestWaitMinutes?: number;
  actions: AutomaticInterventionAction[];
  notificationSettings: {
    sendRealtimePush: boolean;
    sendEmailCphs: boolean;
    sendEmailManagement: boolean;
    triggerAudioChime: 'urgent_alarm' | 'warning_beep' | 'standard_chime' | 'none';
    smsDriverVoucher: boolean;
    titleTemplate: string;
    bodyTemplate: string;
  };
  sanctionRihohsReference?: string;
  updatedAt: string;
  updatedBy: string;
}

export interface ProtocolSimulationStepResult {
  stepNumber: number;
  title: string;
  actionCode: string;
  category: InterventionActionCategory;
  status: 'ejecutado' | 'omitido_desactivado' | 'bloqueo_critico' | 'advertencia';
  detail: string;
  slaMinutes: number;
  timestamp: string;
}
...
export interface ProtocolSimulationResult {
  ...
  totalConfiguredActions: number;
  steps: ProtocolSimulationStepResult[];
  summaryMessage: string;
}

// ==========================================
// CHECKLISTS & INSPECCIONES DE SEGURIDAD VIAL
// ==========================================
...
export type TransportOperationType =
  | 'sustancias_peligrosas' // DS 298 & NCh 382
  | 'mineria_alta_montana'  // Estándar Minero / Sernageomin
  | 'pasajeros_interurbano' // DS 212 MTT
  | 'distribucion_urbana'   // Última Milla / Paquetería
  | 'forestal_ripio'        // Rutas no pavimentadas
  | 'carga_general'         // DS 54 / DS 22 Carretera
  | 'faena_portuaria'       // Terminales portuarios
  | 'cadena_frio';          // Alimentos refrigerados

export type ChecklistItemCriticality =
  | 'CRITICO' // Bloqueo Inmediato de Despacho si está No Conforme
  | 'MAYOR'   // Advertencia formal y plazo máx 24h
  | 'MENOR';  // Observación preventiva

export type ChecklistFieldType =
  | 'conforme_no_conforme' // Conforme / No Conforme / No Aplica
  | 'numerico'             // Valor numérico con unidad y rangos
  | 'fecha'                // Fecha de vigencia
  | 'seleccion'            // Selección entre opciones
  | 'texto';               // Texto libre
...
export interface ChecklistTemplateItem {
  ...
  requiresCommentOnDefect: boolean;
}
...
export interface ChecklistTemplate {
  ...
  name: string;
  description: string;
  applicableVehicleTypes: VehicleCategory[];
  applicableOperations: TransportOperationType[];
  frequency: 'pre_uso_diario' | 'antes_despacho' | 'semanal' | 'mensual' | 'retorno_faena';
  blockingPolicy: 'bloqueo_inmediato_criticos' | 'tolerancia_condicionada';
  maxAllowedMajorDefects: number;
  isOfficialCertified: boolean;
...
export interface ChecklistInspectionAnswer {
  itemId: string;
  status: 'conforme' | 'no_conforme' | 'no_aplica';
  numericValue?: number;
  dateValue?: string;
  textValue?: string;
  observations?: string;
...
export interface ChecklistInspectionExecution {
  id: string;
  folio: string;
  templateId: string;
  templateName: string;
  ...
  answers: Record<string, ChecklistInspectionAnswer>;
  totalItems: number;
  ...
  dispatchDecision: 'AUTORIZADO' | 'CONDICIONADO' | 'BLOQUEO_INMEDIATO';
  dispatchDecisionReason: string;
  driverSignatureTimestamp?: string;
  inspectorSignatureTimestamp?: string;
  hashSha256: string;
  status: 'completada' | 'en_progreso' | 'archivada';
}
