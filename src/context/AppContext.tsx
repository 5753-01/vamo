...
import {
  INITIAL_PREVENTION_PROTOCOLS
} from '../data/preventionProtocolsData';
import {
...
interface AppContextType {
  ...
  // Real-Time Push Notification System for Supervisors (Ley 18.290 / SUSESO)
  realtimePushNotifications: RealtimePushNotification[];
  unreadPushCount: number;
  criticalPushCount: number;
  activeHeadsUpAlert: RealtimePushNotification | null;
  dismissHeadsUpAlert: () => void;
  sendPushNotification: (
    notificationData: Omit<RealtimePushNotification, 'id' | 'timestamp' | 'isRead' | 'isAcknowledged'>
  ) => Promise<RealtimePushNotification | null>;
  acknowledgePushNotification: (id: string, supervisorName: string) => Promise<void>;
  checkLicenseExpirationsNow: () => Promise<RealtimePushNotification[]>;
  
  // Prevention Protocols & Intervention Engine (Ley 18.290 / SUSESO / RIOHS)
  preventionProtocols: PreventionProtocolConfig[];
  updatePreventionProtocol: (protocolId: string, updated: Partial<PreventionProtocolConfig>) => void;
  toggleProtocolAction: (protocolId: string, actionId: string) => void;
  addCustomActionToProtocol: (protocolId: string, action: Omit<AutomaticInterventionAction, 'id'>) => void;
  resetPreventionProtocolsToDefaults: () => void;
  simulateProtocolExecution: (protocolIdOrType: string) => ProtocolSimulationResult;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Prevention Protocols & Intervention Engine (Ley 18.290 / SUSESO / RIOHS)
  const [preventionProtocols, setPreventionProtocols] = useState<PreventionProtocolConfig[]>(() => {
    const saved = localStorage.getItem(`${LOCAL_STORAGE_KEY}_prevention_protocols`);
    return saved ? JSON.parse(saved) : INITIAL_PREVENTION_PROTOCOLS;
  });
  ...
  // Real-Time Push Notifications state for Supervisors (Ley 18.290 / SUSESO)
  const [realtimePushNotifications, setRealtimePushNotifications] = useState<RealtimePushNotification[]>([]);
  const [activeHeadsUpAlert, setActiveHeadsUpAlert] = useState<RealtimePushNotification | null>(null);

  const dismissHeadsUpAlert = () => {
    setActiveHeadsUpAlert(null);
  };

  const sendPushNotification = async (
    notificationData: Omit<RealtimePushNotification, 'id' | 'timestamp' | 'isRead' | 'isAcknowledged'>
  ): Promise<RealtimePushNotification | null> => {
    const result = await pushNotificationService.broadcastNotification(notificationData);
    if (result) {
      setRealtimePushNotifications((prev) => {
        if (prev.some((n) => n.id === result.id)) return prev;
        return [result, ...prev];
      });
      if (result.severity === 'critica') {
        setActiveHeadsUpAlert(result);
      }
    }
    return result;
  };

  const acknowledgePushNotification = async (id: string, supervisorName: string) => {
    await pushNotificationService.acknowledgeNotification(id, supervisorName);
    setRealtimePushNotifications((prev) =>
      prev.map((n) =>
        n.id === id
          ? {
    ...
    if (activeHeadsUpAlert?.id === id) {
      setActiveHeadsUpAlert(null);
    }
  };

  const checkLicenseExpirationsNow = async (): Promise<RealtimePushNotification[]> => {
    const generated = await pushNotificationService.checkLicenseExpirations(drivers);
    if (generated.length > 0) {
      ...
      const firstCritical = generated.find((g) => g.severity === 'critica');
      if (firstCritical) {
        setActiveHeadsUpAlert(firstCritical);
      }
    }
    return generated;
  };

  // Connect WebSocket on user/company change and listen to incoming push notifications
  useEffect(() => {
    ...
    // Fetch initial stored notifications from server
    pushNotificationService.fetchNotifications().then((list) => {
      if (list && list.length > 0) {
        setRealtimePushNotifications(list);
      }
    });

    // Subscribe to real-time incoming push broadcasts
    const unsubscribe = pushNotificationService.subscribe((notification) => {
      ...
      if (notification.severity === 'critica') {
        setActiveHeadsUpAlert(notification);
      }

      // Add to general alerts list if high risk
      if (notification.type === 'high_risk_test' || notification.type === 'license_expired') {
        const newAlert: AlertItem = {
          id: `alt-${notification.id}`,
          companyId: notification.companyId || currentCompany.id,
          type: notification.type === 'high_risk_test' ? 'test_positivo' : 'licencia_vencida',
          severity: 'alta',
          ...
          read: false,
          resolved: false,
          relatedEntityId: notification.testData?.testId || notification.licenseData?.driverId,
          relatedEntityType: notification.type === 'high_risk_test' ? 'test' : 'driver'
        };
  ...
  }, [currentUser.id, currentUser.role, currentCompany.id]);

  const unreadPushCount = realtimePushNotifications.filter((n) => !n.isAcknowledged).length;
  const criticalPushCount = realtimePushNotifications.filter((n) => n.severity === 'critica' && !n.isAcknowledged).length;
  ...
  useEffect(() => {
    ...
    localStorage.setItem(`${LOCAL_STORAGE_KEY}_prevention_protocols`, JSON.stringify(preventionProtocols));
  ...
  const addTestRecord = (testData: Omit<TestRecord, 'id' | 'code' | 'timestamp'>): TestRecord => {
    ...
    if (isPositive) {
      ...
      // Trigger Real-Time Push Notification to Supervisors (WebSocket + Web Audio + Native Push)
      const reactiveDrugsList = (testData.drugPanelResults || [])
        .filter((d) => d.result === 'presunto_positivo' || d.result === 'confirmado_positivo')
        .map((d) => d.name);

      const riskReason =
        testData.alcoholValueGramsPerLiter !== undefined && testData.alcoholValueGramsPerLiter > 0
          ? `Alcohotest Positivo: ${testData.alcoholValueGramsPerLiter.toFixed(2)} g/L (Ley 18.290 Tolerancia Cero)`
          : reactiveDrugsList.length > 0
          ? `Panel Drogas Reactivo: ${reactiveDrugsList.join(', ')}`
          : testData.alcoholStatus === 'rechaza_test'
          ? 'Rechazo injustificado a someterse a control de alcohotest'
          : 'Control de Intemperancia NO CONFORME';

      sendPushNotification({
        type: 'high_risk_test',
        severity: 'critica',
        title: `🚨 TEST ALTO RIESGO: ${testData.driverName} (${code})`,
        body: `Control preventivo en garita detectó resultado NO APTO para el conductor ${testData.driverName} (${testData.driverRut}). ${riskReason}. Despacho bloqueado según Ley 18.290 e ISO 37301.`,
        companyId: currentCompany.id,
        ...
        testData: {
          ...
          driverRut: testData.driverRut,
          driverBase: testData.driverBase,
          vehiclePlate: testData.vehiclePlate,
          operatorName: testData.operatorName,
          alcoholValueGramsPerLiter: testData.alcoholValueGramsPerLiter || 0,
          alcoholStatus: testData.alcoholStatus,
          drugsOverallStatus: testData.drugsOverallStatus,
          reactiveDrugs: reactiveDrugsList,
          riskReason,
  ...
  // Prevention Protocols & Intervention Engine Handlers
  const updatePreventionProtocol = (protocolId: string, updated: Partial<PreventionProtocolConfig>) => {
    setPreventionProtocols((prev) =>
      prev.map((proto) => {
        if (proto.id === protocolId) {
          return {
            ...
            updatedBy: currentUser.name || 'Prevención Corporativa'
          };
        }
        return proto;
      })
    ...
    logAction(
      'PROTOCOLO_CONFIG_MODIFICADO',
      'ProtocoloPrevencion',
      protocolId,
      `Protocolo ${protocolId} actualizado por ${currentUser.name}`
    );
    showToast('Configuración de protocolo actualizada correctamente.');
  };

  const toggleProtocolAction = (protocolId: string, actionId: string) => {
    setPreventionProtocols((prev) =>
      prev.map((proto) => {
        if (proto.id === protocolId) {
          const updatedActions = proto.actions.map((act) => {
            if (act.id === actionId) {
              const newState = !act.enabled;
              return { ...act, enabled: newState };
            }
            return act;
          });
          return {
  ...
  const addCustomActionToProtocol = (
    protocolId: string,
    action: Omit<AutomaticInterventionAction, 'id'>
  ) => {
    const newAction: AutomaticInterventionAction = {
      ...action,
    ...
    setPreventionProtocols((prev) =>
      prev.map((proto) => {
        if (proto.id === protocolId) {
          return {
            ...proto,
            actions: [...proto.actions, newAction],
            updatedAt: new Date().toISOString().replace('T', ' ').substring(0, 19),
            updatedBy: currentUser.name || 'Prevención Corporativa'
          };
        }
    ...
    logAction(
      'ACCION_PROTOCOLO_CREADA',
      'ProtocoloPrevencion',
      protocolId,
      `Acción agregada: ${newAction.name} (${newAction.code})`
    );
    showToast(`Acción "${newAction.name}" incorporada al protocolo.`);
  };

  const resetPreventionProtocolsToDefaults = () => {
    setPreventionProtocols(INITIAL_PREVENTION_PROTOCOLS);
    localStorage.removeItem(`${LOCAL_STORAGE_KEY}_prevention_protocols`);
    logAction(
      'PROTOCOLOS_RESTABLECIDOS',
      'ProtocoloPrevencion',
      'ALL',
      'Restablecidos los 7 protocolos base a estándares Ley 18.290 / SUSESO'
    );
    showToast('Protocolos restablecidos a los valores normativos estándar (Ley 18.290 / SUSESO).');
  };

  const simulateProtocolExecution = (protocolIdOrType: string): ProtocolSimulationResult => {
    const protocol =
      preventionProtocols.find((p) => p.id === protocolIdOrType || p.resultType === protocolIdOrType) ||
    ...
    const steps = protocol.actions.map((action, idx) => {
      let status: 'ejecutado' | 'omitido_desactivado' | 'bloqueo_critico' | 'advertencia' = action.enabled
        ? 'ejecutado'
        : 'omitido_desactivado';

      if (action.enabled && (protocol.riskLevel === 'critico' || action.mandatoryByLaw)) {
        status = 'bloqueo_critico';
      }
      ...
      return {
        ...
        detail: action.enabled
          ? `Ejecutada automáticamente según SLA (${action.slaResolutionMinutes}m). Base legal: ${action.legalReference}. Asignado a rol: ${action.assignedRole}. ${action.requiresSupervisorAck ? '[Requiere Acuse de Supervisor]' : ''}`
          : `Acción desactivada por configuración. Omitida en este ciclo de intervención.`,
        slaMinutes: action.slaResolutionMinutes,
        timestamp: now
      };
    });

    const activeCount = protocol.actions.filter((a) => a.enabled).length;

    // Trigger test push notification if enabled
    if (protocol.notificationSettings.sendRealtimePush && protocol.riskLevel !== 'bajo') {
      sendPushNotification({
        type: 'high_risk_test',
        severity: protocol.riskLevel === 'critico' ? 'critica' : 'alta',
        title: `🧪 SIMULACIÓN PROTOCOLO: ${protocol.title}`,
        body: `Prueba de intervención automática: ${protocol.thresholdCondition}. Se activaron ${activeCount} de ${protocol.actions.length} acciones automatizadas configuradas. Despacho ${protocol.dispatchAllowed ? 'HABILITADO' : 'BLOQUEADO'}.`,
        companyId: currentCompany.id,
        targetRoles: ['supervisor', 'prevencionista', 'company_admin', 'superadmin'],
        targetView: 'prevention_protocols',
        audioChime:
          protocol.notificationSettings.triggerAudioChime !== 'none'
            ? protocol.notificationSettings.triggerAudioChime
            : 'warning_beep'
      });
    }

    logAction(
      'SIMULACION_PROTOCOLO',
      'ProtocoloPrevencion',
      protocol.id,
      `Simulación ejecutada para resultado "${protocol.resultType}". ${activeCount} acciones evaluadas.`
    );
    ...
    return {
      ...
      dispatchAllowed: protocol.dispatchAllowed,
      executedActionsCount: activeCount,
      totalConfiguredActions: protocol.actions.length,
      steps,
      summaryMessage: protocol.dispatchAllowed
        ? `Protocolo Aprobado: ${activeCount} acciones de conformidad ejecutadas. Salida de vehículo autorizada.`
        : `Protocolo Bloqueante Activado: ${activeCount} acciones de intervención ejecutadas. Vehículo retenido y alertas emitidas.`
    };
  };
